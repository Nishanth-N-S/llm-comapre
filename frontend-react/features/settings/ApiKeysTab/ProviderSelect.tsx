...
const ProviderSelect: React.FC<ProviderSelectProps> = ({ providers, selectedProvider, onProviderChange }) => {
  return (
          ...
          >
            {providers.map((p) => (
              <option 
                key={p.provider} 
                value={p.provider}
                className={p.apiKey ? 'text-green-600 dark:text-green-400 font-medium' : ''}
              >
                {p.displayName}
              </option>
            ))}
